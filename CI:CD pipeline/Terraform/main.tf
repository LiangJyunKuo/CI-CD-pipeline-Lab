...
Integrated Ansible with Nexas for automated deployment of application updates, configuration changes, and infrastructure scaling.
Utilized Ansible playbooks for automated infrastructure provisioning and configuration management, ensuring consistency across development, testing, and production environments.
...
/* to enable our Ansible controller node to communicate to the managed node

    => generate the SSH key (ssh-keygen)
    => copy the key to the managed node (ssh-copy-id [IP_address_of_manageNode => ip addr])
*/
...
Managed artifact repositories in Nexus to securely store build artifacts, dependencies, and release versions, seamlessly integrated with Jenkins pipelines for artifact publishing and retrieval across build and deployment stages.
*/
